...
class RelationDataset(Dataset):
    ...
    def __init__(self, data_path, relation_type="subject", vocab=None):
        ...
        for policy in corpus.get("policies", []):
            element     = policy.get("subject" if relation_type == "subject" else "object", "") or ""
            attrs_key   = "subject_attributes" if relation_type == "subject" else "object_attributes"
            for attr in policy.get(attrs_key, []):
                value = attr.get("value", "")
                label = 1 if attr.get("valid", False) else 0
